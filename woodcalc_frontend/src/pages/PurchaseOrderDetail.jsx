...
const STATUS_OPTIONS = Object.keys(STATUS_KEYS)
  ...
  const sendToSupplier = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSendIsError(true)
        setSendMessage(t('common.error') + ' ' + data.error)
      }
    } catch {
      setSendIsError(true)
      setSendMessage(t('common.error') + ' ' + t('purchaseOrderDetail.couldNotReachServer'))
    }
  ...
  const receiveItem = async (lineItemId) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(err.error || t('purchaseOrderDetail.failedToReceive'))
      }
    } catch {}
  }
  ...
  const recordPayment = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPaymentError(data.error || t('purchaseOrderDetail.failedToRecordPayment'))
      }
    } catch {
      setPaymentError(t('purchaseOrderDetail.couldNotReachServer'))
    }
    ...
    <div dir={language === 'ar' ? 'rtl' : 'ltr'} style={{ minHeight: '100vh', background: '#F7F4F0', fontFamily: "'Inter', sans-serif" }}>
      <div style={{ height: 56, background: DARK, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <span onClick={() => navigate('/purchase-orders')} style={{ color: '#fff', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>{t('purchaseOrderList.title')}</span>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: 24 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              ...
              <div style={{ color: '#888', fontSize: 12, marginTop: 8 }}>
                {t('purchaseOrderDetail.ordered')} {po.order_date}{po.expected_delivery_date ? ` · ${t('purchaseOrderDetail.expected')} ${po.expected_delivery_date}` : ''}{po.actual_delivery_date ? ` · ${t('purchaseOrderDetail.received')} ${po.actual_delivery_date}` : ''}
              </div>
              {po.notes && <div style={{ color: '#666', fontSize: 12, marginTop: 8, fontStyle: 'italic' }}>{po.notes}</div>}
              {sendMessage && (
                <div style={{ fontSize: 12, color: sendIsError ? '#c33' : '#3a3', marginTop: 8 }}>{sendMessage}</div>
              )}
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
              {po.is_late && <span style={{ fontSize: 11, color: '#c33', background: '#fee', padding: '3px 10px', borderRadius: 20, fontWeight: 600 }}>{t('purchaseOrderList.late')}</span>}
              ...
              <button onClick={sendToSupplier} disabled={sendingEmail}
                ...
                {sendingEmail ? t('purchaseOrderDetail.sending') : t('purchaseOrderDetail.sendToSupplier')}
              </button>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: 12, padding: 20, marginBottom: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: DARK }}>{t('purchaseOrderDetail.payment')}</h2>
              <div style={{ fontSize: 12, color: '#888', marginTop: 6 }}>
                {t('purchaseOrderDetail.total')} {po.total_amount} · {t('purchaseOrderDetail.paid')} {po.amount_paid} · {t('purchaseOrderDetail.balanceDue')} <strong style={{ color: po.balance_due > 0 ? DARK : '#3a3' }}>{po.balance_due}</strong>
              </div>
              {po.payment_due_date && (
                <div style={{ fontSize: 12, marginTop: 4 }}>
                  {po.is_payment_overdue ? (
                    <span style={{ color: '#c33', background: '#fee', padding: '3px 10px', borderRadius: 20, fontWeight: 600, fontSize: 11 }}>{t('purchaseOrderDetail.overdueLabel')} {po.payment_due_date}</span>
                  ) : (
                    <span style={{ color: '#888' }}>{t('purchaseOrderDetail.due')} {po.payment_due_date}</span>
                  )}
                </div>
              )}
            ...
            {po.balance_due > 0 && (
              <button onClick={() => setShowPayment(true)}
                ...
                {t('purchaseOrderDetail.recordPayment')}
              </button>
            )}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: DARK }}>{t('purchaseOrderDetail.lineItems')}</h2>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: '#888', marginTop: 2 }}>
                      {li.quantity_received} / {li.quantity_ordered} {t('purchaseOrderDetail.receivedOf')} {li.unit_price ? `· ${li.unit_price}/unit` : ''}
                    </div>
                  </div>
                  {li.is_fully_received ? (
                    <span style={{ fontSize: 11, color: '#3a3', background: '#eef7ee', padding: '3px 10px', borderRadius: 20, fontWeight: 600 }}>{t('purchaseOrderDetail.complete')}</span>
                  ) : (
      ...
      {showAddItem && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#fff', borderRadius: 14, padding: 28, width: 400, boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ fontSize: 18, fontWeight: 800, color: DARK, marginBottom: 4 }}>{t('purchaseOrderDetail.addLineItemTitle')}</div>
            <div style={{ fontSize: 12, color: '#888', marginBottom: 20 }}>{t('purchaseOrderDetail.addLineItemDesc')}</div>

            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: '#666', marginBottom: 4, fontWeight: 500 }}>{t('purchaseOrderDetail.fieldMaterial')}</div>
              <select value={itemForm.material} onChange={e => setItemForm(f => ({ ...f, material: e.target.value }))}
                ...
                <option value="">{t('purchaseOrderDetail.selectMaterial')}</option>
                {materials.map(m => <option key={m.id} value={m.id}>{m.sku} — {m.name}</option>)}
              </select>
            </div>

            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: '#666', marginBottom: 4, fontWeight: 500 }}>{t('purchaseOrderDetail.fieldQtyOrdered')}</div>
            ...
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: '#666', marginBottom: 4, fontWeight: 500 }}>{t('purchaseOrderDetail.fieldUnitPrice')}</div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              ...
              <button onClick={addLineItem} disabled={saving || !itemForm.material || !itemForm.quantity_ordered}
                ...
                {saving ? t('purchaseOrderDetail.adding') : t('purchaseOrderDetail.addItem')}
              </button>
            </div>
          </div>
        </div>
      ...
      {showPayment && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#fff', borderRadius: 14, padding: 28, width: 400, boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ fontSize: 18, fontWeight: 800, color: DARK, marginBottom: 4 }}>{t('purchaseOrderDetail.recordPaymentTitle')}</div>
            <div style={{ fontSize: 12, color: '#888', marginBottom: 20 }}>{t('purchaseOrderDetail.balanceDueAmount')} {po.balance_due}</div>

            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: '#666', marginBottom: 4, fontWeight: 500 }}>{t('purchaseOrderDetail.fieldAmount')}</div>
              <input type="number" value={paymentForm.amount} onChange={e => setPaymentForm(f => ({ ...f, amount: e.target.value }))}
                style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E0DAD4', borderRadius: 7, fontSize: 12, outline: 'none', boxSizing: 'border-box', color: DARK }} />
            </div>

            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: '#666', marginBottom: 4, fontWeight: 500 }}>{t('purchaseOrderDetail.fieldMethod')}</div>
              <select value={paymentForm.method} onChange={e => setPaymentForm(f => ({ ...f, method: e.target.value }))}
                style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E0DAD4', borderRadius: 7, fontSize: 12, outline: 'none', boxSizing: 'border-box', color: DARK, background: '#fff' }}>
                <option value="cash">{t('purchaseOrderDetail.methodCash')}</option>
                <option value="bank_transfer">{t('purchaseOrderDetail.methodBankTransfer')}</option>
                <option value="cheque">{t('purchaseOrderDetail.methodCheque')}</option>
              </select>
            </div>

            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: '#666', marginBottom: 4, fontWeight: 500 }}>{t('purchaseOrderDetail.fieldReference')}</div>
              <input type="text" value={paymentForm.reference} onChange={e => setPaymentForm(f => ({ ...f, reference: e.target.value }))}
                placeholder={t('purchaseOrderDetail.referencePlaceholder')}
                style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E0DAD4', borderRadius: 7, fontSize: 12, outline: 'none', boxSizing: 'border-box', color: DARK }} />
            </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
              ...
              <button onClick={recordPayment} disabled={payingSubmitting || !paymentForm.amount}
                ...
                {payingSubmitting ? t('purchaseOrderDetail.recording') : t('purchaseOrderDetail.recordPayment')}
              </button>
            </div>
          </div>
        </div>
