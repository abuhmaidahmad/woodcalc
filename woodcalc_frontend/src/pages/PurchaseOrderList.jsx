    ...
    <div dir={language === 'ar' ? 'rtl' : 'ltr'} style={{ minHeight: '100vh', background: '#F7F4F0', fontFamily: "'Inter', sans-serif" }}>
      <div style={{ height: 56, background: DARK, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <span style={{ color: '#fff', fontWeight: 600, fontSize: 14 }}>{t('purchaseOrderList.title')}</span>
        </div>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: DARK }}>{t('purchaseOrderList.title')}</h1>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: 40, color: '#bbb' }}>
            ...
            <div style={{ fontWeight: 600 }}>{t('purchaseOrderList.emptyTitle')}</div>
            <div style={{ fontSize: 12, marginTop: 4 }}>{suppliers.length === 0 ? t('purchaseOrderList.addSupplierFirst') : t('purchaseOrderList.clickToCreate')}</div>
          </div>
        ) : (
                ...
                <div key={o.id} onClick={() => navigate(`/purchase-orders/${o.id}`)}
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: '#888', marginTop: 2 }}>
                      {o.supplier_name} {o.expected_delivery_date ? `· ${t('purchaseOrderList.expected')} ${o.expected_delivery_date}` : ''}
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    {o.is_late && <span style={{ fontSize: 11, color: '#c33', background: '#fee', padding: '2px 8px', borderRadius: 4 }}>{t('purchaseOrderList.late')}</span>}
                    <span style={{ fontSize: 11, color: st.color, background: st.bg, padding: '3px 10px', borderRadius: 20, fontWeight: 600 }}>{STATUS_KEYS[o.status] ? t(STATUS_KEYS[o.status]) : o.status}</span>
      ...
      {showAdd && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#fff', borderRadius: 14, padding: 28, width: 400, boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ fontSize: 18, fontWeight: 800, color: DARK, marginBottom: 4 }}>{t('purchaseOrderList.modalTitle')}</div>
            <div style={{ fontSize: 12, color: '#888', marginBottom: 20 }}>{t('purchaseOrderList.modalDesc')}</div>

            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: '#666', marginBottom: 4, fontWeight: 500 }}>{t('purchaseOrderList.fieldSupplier')}</div>
              <select value={form.supplier} onChange={e => setForm(f => ({ ...f, supplier: e.target.value }))}
                ...
                <option value="">{t('purchaseOrderList.selectSupplier')}</option>
                {suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
              </select>
            </div>

            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: '#666', marginBottom: 4, fontWeight: 500 }}>{t('purchaseOrderList.fieldExpectedDate')}</div>
            ...
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: '#666', marginBottom: 4, fontWeight: 500 }}>{t('purchaseOrderList.fieldNotes')}</div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              ...
              <button onClick={createOrder} disabled={saving || !form.supplier}
                ...
                {saving ? t('purchaseOrderList.creating') : t('purchaseOrderList.createAndAddItems')}
              </button>
            </div>
          </div>
        </div>
