...
const UNIT_KEYS = {
  PCS: 'materialList.unitPieces', M: 'materialList.unitMeters', M2: 'materialList.unitSquareMeters',
  M3: 'materialList.unitCubicMeters', KG: 'materialList.unitKilograms', L: 'materialList.unitLiters',
}

// Category values are free-text, stored as-is on Material.category (not a fixed backend
// enum like status codes) — kept in English so existing materials' categories keep
// matching regardless of UI language, rather than splitting into English/Arabic variants.
    ...
    <div dir={language === 'ar' ? 'rtl' : 'ltr'} style={{ minHeight: '100vh', background: '#F7F4F0', fontFamily: "'Inter', sans-serif" }}>
      <div style={{ height: 56, background: DARK, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <span style={{ color: '#fff', fontWeight: 600, fontSize: 14 }}>{t('materialList.title')}</span>
        </div>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: DARK }}>{t('materialList.title')}</h1>
            <div style={{ color: '#888', fontSize: 13, marginTop: 2 }}>
              {materials.length} total
              {lowStockCount > 0 && <span style={{ color: '#c33', fontWeight: 600 }}> · {lowStockCount} {t('materialList.lowStock')}</span>}
            </div>
          </div>
          <button onClick={() => setShowAdd(true)}
            ...
            + {t('materialList.newMaterial')}
          </button>
        </div>

        <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder={t('materialList.searchPlaceholder')}
          ...
          <button onClick={() => setLowStockOnly(v => !v)}
            ...
            {lowStockOnly ? t('materialList.showingLowStock') : t('materialList.lowStockOnly')}
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: 40, color: '#bbb' }}>
            ...
            <div style={{ fontWeight: 600 }}>{t('materialList.emptyTitle')}</div>
            <div style={{ fontSize: 12, marginTop: 4 }}>{t('materialList.emptyHint')}</div>
          </div>
        ) : (
              ...
              <div key={m.id}
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#888', marginTop: 2 }}>
                    {m.category ? `${m.category} · ` : ''}{UNIT_KEYS[m.unit] ? t(UNIT_KEYS[m.unit]) : m.unit}{m.unit_cost ? ` · ${m.unit_cost}/unit` : ''}
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  {parseFloat(m.quantity_on_hand) <= parseFloat(m.reorder_level) && (
                    <span style={{ fontSize: 11, color: '#c33', background: '#fee', padding: '2px 8px', borderRadius: 4 }}>{t('materialList.lowStock').toUpperCase()}</span>
                  )}
      ...
      {showAdd && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#fff', borderRadius: 14, padding: 28, width: 420, maxHeight: '85vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ fontSize: 18, fontWeight: 800, color: DARK, marginBottom: 4 }}>{t('materialList.modalTitle')}</div>
            <div style={{ fontSize: 12, color: '#888', marginBottom: 20 }}>{t('materialList.modalDesc')}</div>

            {[['sku', t('materialList.fieldSku')], ['name', t('materialList.fieldName')]].map(([key, label]) => (
            ...
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: '#666', marginBottom: 4, fontWeight: 500 }}>{t('materialList.fieldCategory')}</div>
              {!addingCategory ? (
              ...
              {!addingCategory && form.category && (
                <div style={{ fontSize: 11, color: '#888', marginTop: 4 }}>{t('materialList.selected')} {form.category}</div>
              )}
            </div>

            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: '#666', marginBottom: 4, fontWeight: 500 }}>{t('materialList.fieldUnit')}</div>
              <select value={form.unit} onChange={e => setForm(f => ({ ...f, unit: e.target.value }))}
                style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E0DAD4', borderRadius: 7, fontSize: 12, outline: 'none', boxSizing: 'border-box', color: DARK, background: '#fff' }}>
                {Object.entries(UNIT_KEYS).map(([val, key]) => <option key={val} value={val}>{t(key)}</option>)}
              </select>
            </div>

            <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 11, color: '#666', marginBottom: 4, fontWeight: 500 }}>{t('materialList.fieldQtyOnHand')}</div>
                <input type="number" value={form.quantity_on_hand} onChange={e => setForm(f => ({ ...f, quantity_on_hand: e.target.value }))}
                  style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E0DAD4', borderRadius: 7, fontSize: 12, outline: 'none', boxSizing: 'border-box', color: DARK }} />
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 11, color: '#666', marginBottom: 4, fontWeight: 500 }}>{t('materialList.fieldReorderLevel')}</div>
            ...
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: '#666', marginBottom: 4, fontWeight: 500 }}>{t('materialList.fieldUnitCost')}</div>
              <input type="number" value={form.unit_cost} onChange={e => setForm(f => ({ ...f, unit_cost: e.target.value }))}
                style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E0DAD4', borderRadius: 7, fontSize: 12, outline: 'none', boxSizing: 'border-box', color: DARK }} />
            </div>

            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: '#666', marginBottom: 4, fontWeight: 500 }}>{t('materialList.fieldSupplier')}</div>
              <select value={form.supplier} onChange={e => setForm(f => ({ ...f, supplier: e.target.value }))}
                style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E0DAD4', borderRadius: 7, fontSize: 12, outline: 'none', boxSizing: 'border-box', color: DARK, background: '#fff' }}>
                <option value="">{t('materialList.supplierNone')}</option>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              ...
              <button onClick={saveMaterial} disabled={saving || !form.sku.trim() || !form.name.trim()}
                ...
                {saving ? t('common.saving') : t('materialList.saveMaterial')}
              </button>
            </div>
          </div>
        </div>
