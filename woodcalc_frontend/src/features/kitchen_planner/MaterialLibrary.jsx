...
export default function MaterialLibrary({ onSelect, selectedCode, target }) {
  ...
  const brandData = MATERIAL_DB[brand]
  const filtered = brandData.materials.filter(m => {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5, maxHeight: 280, overflowY: 'auto' }}>
        {filtered.map(mat => {
          ...
          const badge = FINISH_BADGE[mat.finish]
          return (
            <div key={mat.code} onClick={() => onSelect(mat)}
              ...
              {/* Color swatch */}
              <div style={{ height: 36, background: mat.hex, borderBottom: '1px solid rgba(0,0,0,0.06)',
                ...(mat.finish === 'wood' ? { backgroundImage: `repeating-linear-gradient(90deg, transparent, transparent 3px, rgba(0,0,0,0.04) 3px, rgba(0,0,0,0.04) 4px)` } : {}),
                ...(mat.finish === 'gloss' ? { backgroundImage: `linear-gradient(135deg, rgba(255,255,255,0.4) 0%, transparent 50%)` } : {}) }} />
              {/* Info */}
      ...
      {/* Selected display */}
      {selectedCode && (() => {
        const all = Object.values(MATERIAL_DB).flatMap(b => b.materials)
        const found = all.find(m => m.code === selectedCode)
        ...
        return (
          <div style={{ padding: '6px 8px', background: '#F5F0E8', borderRadius: 7, border: `1.5px solid ${ACCENT}33` }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
              <div style={{ width: 20, height: 20, borderRadius: 4, background: found.hex, border: '1px solid #ddd',
                ...(found.finish === 'gloss' ? { backgroundImage: 'linear-gradient(135deg, rgba(255,255,255,0.5) 0%, transparent 60%)' } : {}) }} />
              <div>
